<x-app-layout>
    <x-slot name="header">
        Создание Характеристики Значение "{{$characteristics_value->name}}"
    </x-slot>
    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        ...
        <form method="POST" action="{{ route('admin.characteristics-values.update', $characteristics_value) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            @include('characteristics-values.form')
        </form>
    </div>
</x-app-layout>
