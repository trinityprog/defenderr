<template>
                ...
                <div class="lg:relative lg:pt-[5.9375rem]">
                    ...
                    <div class="w-[30rem] mx-auto mt-4">
                        <select v-model="selectedOption" class="border border-mercury rounded-lg py-3 flex justify-between h-[3.25rem] text-center w-full">
                            <option v-for="size in product.options.size" :key="size.slug" :value="{size: size.slug}">{{ size.name }}</option>
                        </select>
                        <div class="lg:flex justify-center items-start gap-x-5 hidden mt-4">
                            <div class="relative">
    ...
    <section id="description">
        <div class="container">
            <div class="lg:flex items-center justify-between md:hidden gap-x-[6.25rem] mt-[12.5rem]">
                <div class="lg:min-w-[37.5rem] lg:h-[37.5rem] min-w-[21.4375rem] h-[21.4375rem] mx-auto bg-athens-gray bg-cover bg-center bg-no-repeat" :style="{'background-image': `url(/i/images/nitrile_gloves/description.png)`}"></div>
                <div class="px-6 md:px-0">
                    <p class="text-cape-cod text-[2.0625rem] pb-10 pt-[3.125rem] px-6 md:px-0">{{ product.shortName }}</p>
                    <div class="px-6 md:px-0" v-html="$t('nitrile_gloves.description')"></div>
                </div>
            </div>
        </div>
    </section>
...
const selectedOption = ref({size: _.find(product.value.options.size, i => i.product.id === product.value.id).slug});
const getProductByOption = computed(() => _.find(product.value.options.size, ['slug', selectedOption.value.size]).product)
